---
name: git-master
description: "MUST USE for ANY git operations. Atomic commits, rebase/squash, history search (blame, bisect, log -S). STRONGLY RECOMMENDED: Use with kraken_task(category='quick', skills=['git-master'], ...) to save context. Triggers: 'commit', 'rebase', 'squash', 'who wrote', 'when was X added', 'find the commit that'."
---
...
**Check opencode.json for these flags:**
...
If enabled, add Kraken attribution to EVERY commit:

1. **Footer in commit body (if `commit_footer: true`):**
```
Ultraworked with [Kraken](https://github.com/leviathofnoesia/kraken-code)
```

2. **Co-authored-by trailer (if `include_co_authored_by: true`):**
```
Co-authored-by: Kraken <kraken-agent@krakencode.dev>
```

**Example (both enabled):**
```bash
git commit -m "{Commit Message}" -m "Ultraworked with [Kraken](https://github.com/leviathofnoesia/kraken-code)" -m "Co-authored-by: Kraken <kraken-agent@krakencode.dev>"
```

**To disable:** Set in opencode.json:
```json
