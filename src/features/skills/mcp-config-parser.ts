...
export function validateMcpConfig(config: any): SkillMcpConfig | null {
  ...
  for (const [serverName, serverConfig] of Object.entries(config)) {
    ...
    if (
      ...
      !("command" in configObj)
    ) {
      console.warn(
        `[mcp-config-parser] Invalid MCP config for server ${serverName}`
      )
      continue
    }
    ...
    const args = Array.isArray(configObj.args)
      ...
      : []
    const env = typeof configObj.env === "object"
      ? configObj.env as Record<string, string>
      : undefined
    const timeout = typeof configObj.timeout === "number"
      ? configObj.timeout
      : 30000
