  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('ENOENT')) {
      return {
        success: false,
        error:
          'ripgrep (rg) not installed. Install with: apt install ripgrep or brew install ripgrep',
      }
    }
    return {
      success: false,
    ...
    return JSON.stringify(
      {
        ...
        count: result.count,
        message:
          result.count && result.count > 100 ? `Showing 100 of ${result.count} matches` : undefined,
      },
      null,
      2,
    )
