...
async function showStatus(worktree: string): Promise<Record<string, any>> {
  ...
  const fullConfig = JSON.parse(await readFile(configPath, 'utf-8')) as {
    modelSwitcher?: { setupComplete?: boolean }
  }
  ...
  return {
    agents,
    builtInPresets,
    customPresets: Object.keys(presets).filter(
      (p) => !['performance', 'quality', 'balanced', 'economy'].includes(p),
    ),
