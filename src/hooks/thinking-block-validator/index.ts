...
function validateThinkingBlocks(
  ...
  if (config.enforcePosition) {
    const lastThinkingClose = closeTags.length > 0
      ? Math.max(...closeTags)
      : -1;
    const hasContentAfter = lastThinkingClose >= 0
      ? text.substring(lastThinkingClose).trim().length > 0
      : false;
    ...
    'chat.message': async (input, output) => {
      ...
      if (validation.hasErrors) {
        ...
        for (const error of validation.errors) {
          ...
          console.log(
            `[thinking-block-validator] Suggestion: ${error.suggestion}`
          );

          if (error.recovered) {
            console.log(
              `[thinking-block-validator] ✓ Auto-recovery applied`
            );
          }
        }
        ...
        if (config.autoRecover && validation.recovered) {
          ...
          console.log(
            '[thinking-block-validator] [session recovered - continuing previous task]'
          );
          console.log(
