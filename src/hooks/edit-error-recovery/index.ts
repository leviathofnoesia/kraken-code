...
type EditErrorType = 'file_not_found' | 'permission_denied' | 'merge_conflict' | 'concurrent_edit' | 'unknown';
...
async function applyRecovery(
  ...
  if (!action.canRecover) {
    console.log(
      `[edit-error-recovery] Cannot recover from error type: ${errorType}`
    );
    console.log(
      `[edit-error-recovery] Suggestion: ${action.suggestion}`
    );
    return false;
  }

  console.log(
    `[edit-error-recovery] Attempting recovery action: ${action.action}`
  );
  console.log(
    `[edit-error-recovery] Suggestion: ${action.suggestion}`
  );
  ...
  if (errorType === 'file_not_found' && config.autoCreateDirs) {
    try {
      ...
      if (dir) {
        ...
        console.log(
          `[edit-error-recovery] Created directory: ${dir}`
        );
      }
      return true;
    } catch (e) {
      console.error(
        `[edit-error-recovery] Failed to create directory: ${e}`
      );
      return false;
    }
  }
    ...
    'tool.execute.after': async (input, output) => {
      ...
      if (outputAny.error || outputAny.status === 'error') {
        ...
        if (currentRetries < maxRetries) {
          ...
          if (filePath) {
            console.log(
              `[edit-error-recovery] File: ${filePath}`
            );
          }
          console.log(
            `[edit-error-recovery] Error type: ${errorType}`
          );
          console.log(
            `[edit-error-recovery] Error: ${error}`
          );
          ...
          if (recovered) {
            ...
            console.log(
              `[edit-error-recovery] Retry scheduled in ${delay}ms`
            );
            console.log(
              '[edit-error-recovery] [session recovered - continuing previous task]'
            );
            ...
            console.error(
              `[edit-error-recovery] Recovery failed for ${filePath || 'unknown'}`
            );
            editRetries.delete(retryKey);
          }
        } else {
...
export function getFileEditHistory(
  filePath: string
