...
export function getSoundPath(event: SoundEvent, config: SoundConfig = { enabled: true }): string | null {
  if (!config.enabled) {
    return null
  }
...
export async function playSound(
  ...
  const platform = getPlatform()
  const platformCommand = PLATFORM_SOUND_COMMANDS !== null ? PLATFORM_SOUND_COMMANDS[platform as keyof typeof PLATFORM_SOUND_COMMANDS] : null
  ...
  try {
    ...
    if (platform === "win32") {
      ...
      const volumeArg = config.volume !== undefined && config.volume !== 1.0
...
export function testSoundPlayback(): {
  ...
  const command = PLATFORM_SOUND_COMMANDS !== null ? PLATFORM_SOUND_COMMANDS[platformName as keyof typeof PLATFORM_SOUND_COMMANDS] : null
